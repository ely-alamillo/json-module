...
const jsonModule_1 = require("../src/jsonModule");
const assert = chai.assert;
describe('Singleton', () => {
    let easy;
    before(done => {
        const destination = `${root.path}/config`;
        easy = jsonModule_1.jsonModule.getInstance('config.json', destination);
        done();
    });
    ...
    it('Fails on double instantiations', () => {
        assert.throw(() => {
            try {
                let myJSON = new jsonModule_1.jsonModule();
            }
            catch (e) {
                throw e;
            }
        }, Error, "Yikes! You can't create this object. Try jsonModule.getInstance()");
    });
});
